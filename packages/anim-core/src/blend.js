...
function slerpQuaternion(ax, ay, az, aw, bx, by, bz, bw, t) {
    ...
    if (cosHalfTheta > 0.9995) {
        return normalizeQuaternion(lerp(ax, endX, t), lerp(ay, endY, t), lerp(az, endZ, t), lerp(aw, endW, t));
    }
    ...
    return normalizeQuaternion(ax * ratioA + endX * ratioB, ay * ratioA + endY * ratioB, az * ratioA + endZ * ratioB, aw * ratioA + endW * ratioB);
}
...
export function blendPoses(a, b, weight, out) {
    ...
    for (let boneIndex = 0; boneIndex < out.boneCount; boneIndex += 1) {
        ...
        const [qx, qy, qz, qw] = slerpQuaternion(a.rotations[quatOffset], a.rotations[quatOffset + 1], a.rotations[quatOffset + 2], a.rotations[quatOffset + 3], b.rotations[quatOffset], b.rotations[quatOffset + 1], b.rotations[quatOffset + 2], b.rotations[quatOffset + 3], t);
...
export function blendPosesMasked(base, overlay, globalWeight, mask, out) {
    copyPose(base, out);
    for (let boneIndex = 0; boneIndex < out.boneCount; boneIndex += 1) {
        ...
        const [qx, qy, qz, qw] = slerpQuaternion(base.rotations[quatOffset], base.rotations[quatOffset + 1], base.rotations[quatOffset + 2], base.rotations[quatOffset + 3], overlay.rotations[quatOffset], overlay.rotations[quatOffset + 1], overlay.rotations[quatOffset + 2], overlay.rotations[quatOffset + 3], weight);
        out.rotations[quatOffset] = qx;
        out.rotations[quatOffset + 1] = qy;
        out.rotations[quatOffset + 2] = qz;
        out.rotations[quatOffset + 3] = qw;
...
export function addPoseAdditive(base, additive, rig, globalWeight, mask, out) {
    copyPose(base, out);
    for (let boneIndex = 0; boneIndex < out.boneCount; boneIndex += 1) {
        ...
        const bindInverse = invertQuaternion(rig.bindRotations[quatOffset], rig.bindRotations[quatOffset + 1], rig.bindRotations[quatOffset + 2], rig.bindRotations[quatOffset + 3]);
        const delta = multiplyQuaternion(additive.rotations[quatOffset], additive.rotations[quatOffset + 1], additive.rotations[quatOffset + 2], additive.rotations[quatOffset + 3], bindInverse[0], bindInverse[1], bindInverse[2], bindInverse[3]);
        const weightedDelta = slerpQuaternion(0, 0, 0, 1, delta[0], delta[1], delta[2], delta[3], weight);
        const result = multiplyQuaternion(weightedDelta[0], weightedDelta[1], weightedDelta[2], weightedDelta[3], base.rotations[quatOffset], base.rotations[quatOffset + 1], base.rotations[quatOffset + 2], base.rotations[quatOffset + 3]);
