...
function sampleScalarTriplet(times, values, time, out) {
    if (times.length === 1) {
        out[0] = values[0];
        out[1] = values[1];
        out[2] = values[2];
...
function sampleTrackChannel(track, time, pose) {
...
export function normalizeClipTime(clip, time, loop) {
    if (clip.duration <= 0) {
        return 0;
    }
    if (!loop) {
...
export function estimateClipDuration(clip) {
    return clip.duration;
}
export function sampleClipRootMotionDelta(clip, rig, previousTime, nextTime, mode) {
