...
function NodeInspector({ store }: { store: AnimationEditorStore }) {
  ...
  if (!graph || !node) {
    return <div style={{ color: "#667085", fontSize: 13 }}>Select a node to edit its properties.</div>;
  }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
          ...
          <Panel title="Diagnostics">
            {state.diagnostics.length === 0 ? (
              <div style={{ color: "#667085", fontSize: 13 }}>No diagnostics yet. Compile to validate the document.</div>
            ) : (
